...
interface NavItem {
  id: ViewType; labelKey: string; icon: React.ReactNode; badge?: string; color?: string;
}
        ...
        <AnimatePresence>
          {!sidebarCollapsed && (
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}
              transition={{ duration: 0.2 }} className="flex items-center gap-2.5 min-w-0"
            >
              <div className="relative flex-shrink-0">
                ...
                {isPlaying && <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />}
              </div>
              <div className="min-w-0">
                <div className="text-sm font-bold tracking-wider" style={{ background: 'linear-gradient(135deg, #a78bfa, #7c3aed)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>NEUROTEK</div>
        ...
        {!sidebarCollapsed && activeProject && (
          ...
          >
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: activeProject.coverColor.includes('gradient') ? '#7c3aed' : activeProject.coverColor }} />
              <div className="min-w-0">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => setView(item.id)}
              className={clsx('w-full flex items-center rounded-lg transition-all duration-150 group',
                sidebarCollapsed ? 'justify-center p-2.5' : 'gap-3 px-3 py-2.5',
                isActive ? 'text-white' : 'text-text-secondary hover:text-text-primary'
              )}
              style={isActive ? { background: `${item.color}18`, border: `1px solid ${item.color}35`, boxShadow: `0 0 12px ${item.color}15`, color: item.color } : { border: '1px solid transparent' }}
              title={sidebarCollapsed ? t(item.labelKey) : undefined}
            >
              <span className="flex-shrink-0 transition-colors" style={isActive ? { color: item.color } : {}}>{item.icon}</span>
              <AnimatePresence>
                {!sidebarCollapsed && (
      ...
      <div className="p-3 space-y-2" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
        {!sidebarCollapsed ? (
          <>
            {auth.user && (
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleLogout} className="flex-shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-text-muted hover:text-red-400 hover:bg-red-900/20 transition-colors" title="Sign out">
                  <LogOut size={14} />
                </button>
              </div>
            )}
            <div className="flex items-center justify-between text-[10px] text-text-muted">
              <div className="flex items-center gap-1.5"><Zap size={10} className="text-amber-400" /><span>AI Engine</span></div>
              <div className="flex items-center gap-1"><Circle size={8} className="fill-emerald-400 text-emerald-400" /><span className="text-emerald-400">Online</span></div>
            </div>
            <div className="flex items-center justify-between text-[10px] text-text-muted">
              <span>Latency</span>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <button onClick={handleLogout} className="w-7 h-7 rounded-lg flex items-center justify-center text-text-muted hover:text-red-400 hover:bg-red-900/20 transition-colors" title="Sign out">
              <LogOut size={14} />
            </button>
          </div>
        )}
